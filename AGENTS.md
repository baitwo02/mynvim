...
## 当前插件面
- 当前真正装配的插件包括：`blink.cmp`、`catppuccin`、`mini.nvim`、`noice.nvim`、`nvim-notify`、`which-key.nvim`、`pi.nvim`、`flash.nvim`、`fzf-lua`、`nvim-treesitter`、`indent-blankline.nvim`，以及 `noice.nvim` 依赖的 `nui.nvim`；不要假设仓库里已经有 `telescope`、`gitsigns`、`nvim-tree` 等常见插件。
...
- `mini.nvim` 配在 `lua/plugins/mini.lua`，当前只启用了 `mini.statusline`；以后如果继续加 `mini` 模块，优先仍放在这个插件模块里集中配置。
...
- `which-key.nvim` 配在 `lua/plugins/which_key.lua`，直接 `require("which-key").setup(...)`；现有快捷键提示主要依赖各映射上的 `desc`，同时给 `<leader>a` / `<leader>c` / `<leader>s` / `<leader>sn` 配了分组名。改动键位时别漏掉同步描述和分组。
